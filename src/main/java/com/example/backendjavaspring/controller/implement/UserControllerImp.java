...
public class UserControllerImp implements UserController {
    Logger logger = LoggerFactory.getLogger(UserControllerImp.class);
    @Autowired
    ...
    public ResponseEntity<?> addUser(UserRequestDTO userRequest) {
        try {
            ...
            if (userService.isUserEmailExist(userRequest.getEmail())) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("TẠO TÀI KHOẢN THẤT BẠI ĐỊA CHỈ EMAIL KHÔNG ĐÚNG ĐỊNH DẠNG!");
            }
